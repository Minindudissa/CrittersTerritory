import { registerForNewsLetter, sendEmail } from "@/services";
...
function Footer() {
  ...
  const emailContent =
    '    <div style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 20px; border-radius: 8px; border: 1px solid #e0e0e0;">' +
    '<div style="background-color: #FFC107; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; color: #000000;">' +
    '<h1 style="margin: 0; font-size: 36px;">Welcome to Critters Territory!</h1>' +
    "</div>" +
    '<div style="color: #333333; padding: 20px; text-align: center;">' +
    '<p style="font-size: 16px;">Hello,</p>' +
    '<p style="font-size: 16px;">Thank you for subscribing to our newsletter! You have successfully registered to receive updates, special offers, and more from Critters Territory.</p>' +
    '<p style="font-size: 16px;">We’re excited to have you with us on this journey. Stay tuned for the latest news and awesome 3D printed creations!</p>' +
    '<a href="https://www.crittersterritory.com" style="background-color: #FFC107; color: #000000; padding: 12px 20px; text-align: center; display: inline-block; font-size: 16px; margin-top: 20px; border-radius: 5px; text-decoration: none;">Visit Critters Territory</a>' +
    "</div>" +
    '<div style="background-color: #333333; color: #ffffff; padding: 15px; text-align: center; font-size: 14px; border-radius: 0 0 8px 8px;">' +
    '<p>If you have any questions, feel free to contact us at <a href="mailto:crittersterritory@gmail.com" style="color: #FFC107; text-decoration: none;">crittersterritory@gmail.com</a>.</p>' +
    "<p>Critters Territory &copy; . All rights reserved.</p>" +
    "</div>" +
    "</div>";
  ...
  async function handleOnClick(event) {
    ...
    if (email === null) {
      ...
      try {
        const data = await registerForNewsLetter({
          email,
        ...
        if (data?.success) {
          setErrorMsg(null);
          if (
            data?.message !== "You have already registered to our newsletters !"
          ) {
            // Email
            ...
            const sendEmailResponse = await sendEmail({
              toName: "User",
              toEmail: email,
              subject: "Registered to Critters Territory Newsletters",
  ...
  return (
    <footer className="bg-black bg-opacity-60 pt-12 pb-8 px-10 font-sans tracking-wide">
      <div className="max-w-screen-xl mx-auto">
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8 flex justify-center">
          <div className=" flex justify-center text-center">
            <ul className="space-y-4">
              <span className="text-yellow-500 text-2xl mb-6 font-bold">
                Critters Territory
              </span>
              <li>
                <a onClick={() => navigate("../", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Home
                </a>
              </li>
              <li>
                <a onClick={() => navigate("../shop", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Shop
                </a>
              ...
              <li>
                <a onClick={() => navigate("../contact-us", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Contact Us
                </a>
              ...
              <li className="text-lg">
                <a onClick={() => navigate("../about-us", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  About Us
                </a>
          ...
          <div className=" flex justify-center text-center">
            <ul className="space-y-4">
              <span className="text-white text-2xl mb-6 font-bold">
                Account
              </span>
              <li>
                <a onClick={() => navigate("../user-orders", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  My Orders
                </a>
              ...
              <li>
                <a onClick={() => navigate("../cart", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Cart
                </a>
              ...
              <li className="text-lg">
                <a onClick={() => navigate("../wishlist", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Wishlist
                </a>
              ...
              <li>
                <a onClick={() => navigate("../voucher", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Gift Vouchers
                </a>
          ...
          <div className=" flex justify-center text-center">
            <ul className="space-y-4">
              ...
              <li>
                <a onClick={() => navigate("../t&c", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Terms & Conditions
                </a>
              ...
              <li>
                <a onClick={() => navigate("../privacy-policy", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Privacy Policy
                </a>
              ...
              <li>
                <a onClick={() => navigate("../delivery", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
                  Delivery
                </a>
              ...
              <li className="text-lg">
                <a onClick={() => navigate("../warranty", { replace: true })}
                  className="text-gray-300 cursor-pointer hover:text-yellow-500 text-lg"
                >
        ...
        <div className="grid max-md:grid-cols-1 md:grid-cols-2 items-center gap-8">
          <div className="max-w-lg">
            ...
            <div className="bg-white flex px-1 py-1 rounded-full text-left">
              <input
                ...
                className="w-full outline-none bg-transparent pl-4 text-lg"
                onChange={(event) => setEmail(event.target.value)}
              />
              <button
                type="button"
                className="bg-yellow-400 hover:bg-yellow-300 hover:border-transparent outline-none focus:outline-none focus:outline-transparent text-black font-bold  text-lg rounded-full px-4 py-2 tracking-wide"
          ...
          <ul className="flex flex-wrap items-center md:justify-end gap-x-6 gap-4">
            <li>
              <a
