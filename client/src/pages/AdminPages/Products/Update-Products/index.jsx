...
function UpdateProducts() {
    ...
    async function fetchData() {
      try {
        ...
        if (productSearchResponse?.success) {
          ...
          setColorCount(
            productSearchResponse.productList[0].colorCount || null
          );
          setDimension(productSearchResponse.productList[0].dimension || null);

          setProductType(productSearchResponse.productList[0].productType);
          setTitle(productSearchResponse.productList[0].title);
          setDescription(productSearchResponse.productList[0].description);
          setMaterial(productSearchResponse.productList[0].material || null);
          setWeight(productSearchResponse.productList[0].weight || null);
          setPrintSettings(
            productSearchResponse.productList[0].printSettings || null
          );
          setBasePrice(productSearchResponse.productList[0].basePrice || null);
          setStockType(productSearchResponse.productList[0].stockTypeId || "0");
          setCategory(productSearchResponse.productList[0].categoryId || "0");
          setSharableLink(
            productSearchResponse.productList[0].sharableLink || null
          );
          setStockCount(productSearchResponse.productList[0].stock || null);
          setPrinterType(
            productSearchResponse.productList[0].printerTypeId || "0"
          );
          setColorFileAvailability(
            productSearchResponse.productList[0].isColorFileAvailableId || "0"
          );
          if (productSearchResponse.productList[0].productType === "Physical") {
            const productData = productSearchResponse.productList[0];
            const { variationsList = [], variations = [] } = productData;

            const optionSets = variations.reduce((acc, item) => {
            ...
            const parsedVariations = variationsList.map((name, index) => ({
              name,
            ...
            productSearchResponse.productList[0]?.variationsList.map(
              (variationItem, Index) =>
                variationItem === "Color"
                  ? setColorVariationIndex(Index)
                  : variationItem === "Size"
          ...
          setCombinations(
            productSearchResponse.productList[0].variations || []
          );
        }
  ...
  async function handleProductStatusOnSubmit(event) {
    event.preventDefault();

    const actionText = productList.status === "1" ? "deactivate" : "activate";
    ...
    try {
      const newStatus = productList.status === "1" ? "0" : "1";

      const productUpdateResponse = await productUpdate({
        updateSelectData: { _id: id },
      ...
      if (productUpdateResponse?.success) {
        ...
        console.error("Update failed:", productUpdateResponse?.message);
      }
    } catch (error) {
  ...
  async function handleUpdateProductOnSubmit(event) {
    event.preventDefault();
          ...
          if (isNaN(basePrice)) {
            ...
            if (productUpdateResponse?.success) {
              ...
              if (updateProductImageResponse) {
                ...
                setSuccessMsg(null);
                setErrorMessage(updateProductImageResponse?.message);
              }
            } else {
              setSuccessMsg(null);
              setErrorMessage(productUpdateResponse?.message);
            }
          }
        }
      }
  ...
  return isLoading ? (
    ...
    <div className=" w-full flex flex-col justify-center items-center p-4">
      ...
      <div className=" bg-white bg-opacity-15 rounded-md flex flex-row items-center px-4 w-full">
        <div className="w-full flex flex-row gap-6 py-4"></div>
        <div className="w-full flex flex-row justify-end gap-4">
          {productList?.status === "1" ? (
            <button
              onClick={handleProductStatusOnSubmit}
              className=" my-1 bg-red-500 text-white hover:border-none border-none outline-none focus:outline-transparent focus:outline-none ring-0 focus:ring-0 hover:bg-red-600 border-black border-2 focus:right-0 focus:border-none"
            >
              Inactive
            </button>
          ) : (
            <button
              onClick={handleProductStatusOnSubmit}
              className=" my-1 bg-green-400 text-black hover:border-none border-none outline-none focus:outline-transparent focus:outline-none ring-0 focus:ring-0 hover:bg-green-500 border-black border-2 focus:right-0 focus:border-none"
            >
              Active
            </button>
          )}
          <button
            onClick={handleUpdateProductOnSubmit}
            className=" my-1 bg-yellow-400 hover:border-none border-none outline-none focus:outline-transparent focus:outline-none ring-0 focus:ring-0 hover:bg-yellow-500 border-black border-2 focus:right-0 focus:border-none"
          >
            Update Product
          </button>
        </div>
      ...
      <div className="w-full flex lg:flex-row flex-col-reverse justify-center gap-4 py-4">
        <div className=" lg:w-2/3 w-full flex flex-col space-y-4">
          <div className="  bg-white bg-opacity-15 w-full rounded-md p-4 space-y-4">
            ...
            <div className="w-full flex lg:flex-row flex-col flex-wrap lg:justify-start ">
              {productType === "Physical" ? (
                <>
                        ...
                        index === colorVariationIndex ? (
                          <div className="w-full flex flex-row justify-between flex-wrap">
                            {colorList.map((colorItem, colorIndex) => {
                              const isChecked = variation.options.includes(
                                colorItem.name
                              );
                              return (
                                <div
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        ...
                                        const variationIndexToRemove =
                                          variation.options.indexOf(
                                            colorItem.name
                                          );
                                        removeOption(
                                          index,
                          ...
                          // fallback to manual entry input UI
                  ...
                  <div className="w-full rounded-md  bg-white bg-opacity-15 py-4">
                    <div className="mb-4">
                  ...
                  <div className="w-full">
                    ...
                    <div className="relative flex-col items-center ">
                      ...
                      <select
                        ...
                        value={colorFileAvailability}
                        className="text-gray-800 bg-white border border-gray-300 w-full text-sm px-4 py-2.5 rounded-md outline-yellow-500"
                      >
                        <option value="0">
                          Select Color File Availability
                        </option>
        ...
        <div className="  lg:w-1/3 w-full  bg-white bg-opacity-15 rounded-md">
          <div className=" w-full rounded-md p-4">
            ...
            <div className="flex items-center justify-center w-full">
              <label
                ...
                className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer dark:hover:bg-gray-800 dark:bg-gray-700 hover: bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500"
              >
