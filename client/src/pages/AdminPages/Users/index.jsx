...
function Users() {
  ...
  useEffect(() => {
    async function searchAllUsers(searchData, pagination) {
      const searchAllUsersResponse = await searchUser(searchData, pagination);
      setIsLoading(!searchAllUsersResponse);

      if (searchAllUsersResponse?.success) {
        setAllUsersList(searchAllUsersResponse?.userData);
        setCurrentPage(searchAllUsersResponse?.currentPage);
        setTotalPages(searchAllUsersResponse?.totalPages);
      }
    }
    if (searchTerm === "") {
      searchAllUsers({
  ...
  const toggleAccordion = async (id, genderId) => {
    if (openAccordion === id) {
      ...
      const searchAddressResponse = await searchAddress({
        searchData: { userId: id },
      });
      if (searchAddressResponse?.success) {
        setAddressData(searchAddressResponse?.data[0]);
  ...
  async function handleAccountStatusOnClick(email) {
    const userSearchResponse = await searchUser({ searchData: { email } });
    if (userSearchResponse?.success) {
      if (userSearchResponse?.userData[0].accountStatus === 1) {
        const userUpdateResponse = await UpdateUser({
          email,
          updateData: { accountStatus: 2 },
        });
      ...
      } else if (userSearchResponse?.userData[0].accountStatus === 2) {
        const userUpdateResponse = await UpdateUser({
          email,
  ...
  return isLoading ? (
    ...
    <div className="w-full flex flex-col justify-center items-center">
      <div className="w-full flex flex-col justify-center items-center">
        ...
        <span className="font-bold text-[45px] text-white">Users</span>
      </div>
      ...
      <div className="w-full divide-y rounded-lg max-w-7xl mx-auto p-4 my-8">
        ...
        {allUsersList.map((singleUser) => (
          <div
            ...
            onClick={() => toggleAccordion(singleUser._id, singleUser.genderId)}
          >
            <button
              type="button"
              ...
              <div className="flex flex-col space-y-4">
                ...
                <label className="inline-flex items-center me-5 cursor-pointer">
                  <input
                    type="checkbox"
                    onClick={() =>
                      handleAccountStatusOnClick(singleUser?.email)
                    }
                    className="sr-only peer"
                    defaultChecked={singleUser?.accountStatus === 1}
                    disabled={singleUser?.accountStatus === 0}
                  />
                  ...
                  <span className="ms-3 text-sm font-medium text-gray-200 dark:text-gray-300">
                    {singleUser?.accountStatus === 1
                      ? "Active"
                      : singleUser?.accountStatus === 2
                        ? "Suspended"
                        : "Deleted Account"}
                  </span>
                </label>
            ...
            {openAccordion === singleUser._id && (
              <div className="pt-3 px-3 bg-white bg-opacity-10 w-full flex md:flex-row flex-col md:justify-center">
                <div className=" w-full space-y-3 py-4 ps-4">
                  ...
                  <p className="text-md text-gray-300 overflow-hidden">
                    &#11050;&nbsp;
                    {singleUser.emailVerifiedDateTime}
                  </p>
                  <p className="text-md text-gray-300 overflow-hidden">
                    &#11050;&nbsp;
                    {singleUser.accountStatus === 1
                ...
                <div className=" w-full space-y-3 py-4 ps-4">
                  <p className="text-2xl font-semibold text-gray-300 overflow-hidden">
                    {singleUser.firstName}'s Address
                  </p>
                  <p className="text-md text-gray-300 overflow-hidden">
                    &#11050;&nbsp;
                    {addressData?.line1 || "------------------------------"}
